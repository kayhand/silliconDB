...
public:
	...
	void initializeCompressor(string fileName, int t_id, int agg_id,
			Partitioner &part, int sf) {
		DataCompressor *compressedTable = new DataCompressor(fileName, t_id,
				agg_id, part, sf);
	...
	void parseTable(int t_id) {
		...
		if (t_id == 0) {
			...
			column* datePKCol = &(dataCompressors[3]->getTable()->columns[0]);
			dataCompressors[t_id]->parseFactTable(suppPKCol, custPKCol, datePKCol);
		} else
