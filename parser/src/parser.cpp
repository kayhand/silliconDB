...
table *Parser::parse(){
		...
		for(int col = 0; col < t.nb_columns ; col++){
			...
			switch(t.columns[col].data_type){
				...
				case column::data_type_t::DOUBLE:
					...
					if(current->d_keys.find(d_value) ==
							current->d_keys.end())
	...
	for(int col=0; col < t.nb_columns ; col++){
		switch(t.columns[col].data_type){
			...
			case column::data_type_t::INT:
				...
				for(map<int,int>::iterator iter = t.columns[col].i_keys.begin();
						iter != t.columns[col].i_keys.end();
						iter++)
			...
			case column::data_type_t::DOUBLE:
				...
				for(map<double,int>::iterator iter = t.columns[col].d_keys.begin();
						iter != t.columns[col].d_keys.end();
						iter++)
	...
	//C++ is supposed to do this on it's own
	//file.close();
	return &t;
}
